...
@implementation ListWidget

+(void)load{
    [WidgetMap bind: @"list" withModelClassName: @"ListM" withWidgetClassName: @"ListWidget"];
}
...
- (void) setDataProvider: (id) object {
    ...
    for (NSDictionary *item in items) {
        ...
        if ([((ListM *) self.model).groupBy isKindOfClass: [NSString class]]) {
            section = [item valueForKey: ((ListM *) self.model).groupBy];
        }
