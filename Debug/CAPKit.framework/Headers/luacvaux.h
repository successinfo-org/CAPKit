...
#define LUACV_VERSION_MAJOR 2
#define LUACV_VERSION_MINOR 4
#define LUACV_VERSION_SUBMINOR 9
...
#define checkint(L,narg)\
  (int)luacv_checkint(L,narg,f_msg)
...
void laucv_sigcatcher(lua_State *L);


extern const luaL_Reg luacv_m[];



bool luacv_compare(std::vector<std::string>&,std::vector<std::string>&);
std::vector<std::string>& luacv_getnames(lua_State *);
