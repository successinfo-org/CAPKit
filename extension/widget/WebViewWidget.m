...
+(void)load{
    [WidgetMap bind: @"webview" withModelClassName: @"WebViewM" withWidgetClassName: @"WebViewWidget"];
}
...
-(id)initWithModel:(WebViewM *)m withPageSandbox: (PageSandbox *) sandbox{
...
-(void)onCreated{
    ...
    if (firstLoad) {
        WebViewM *wm = (WebViewM *) self.model;
        if ([wm.src length] > 0) {
...
- (void) setSrc: (NSString *) urlString{
    ...
    ((WebViewM *) self.model).src = urlString;
...
- (NSString *) getSrc{
    return ((WebViewM *) self.model).src;
}
    ...
    if (usingJit) {
        ...
        dispatch_block_t blk = ^{
            ...
            @autoreleasepool {
                ...
                wrapper = [[LuaImage alloc] initWithImage: image];
            }
