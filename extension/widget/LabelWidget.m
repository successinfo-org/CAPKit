...
+(void)load{
    [WidgetMap bind: @"label" withModelClassName: @"LabelM" withWidgetClassName: @"LabelWidget"];
}
    ...
    @autoreleasepool {
        if (self.model.wrap && !self.model.html && self.model.text) {
            ...
            if (self.model.maxLines > 0) {
                height = ((LabelM *) self.model).maxLines * font.lineHeight;
