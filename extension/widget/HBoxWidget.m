...
@implementation HBoxWidget

+(void)load{
    [WidgetMap bind: @"hbox" withModelClassName: @"HBoxM" withWidgetClassName: @"HBoxWidget"];
}
...
-(CGRect)measureRectAbsoluteChildren:(CGSize)parentContentSize{
    ...
    dispatch_block_t measure = ^{
        ...
        for (int i = 0; i < [subitems count]; i++) {
            AbstractUIWidget *widget = [subitems objectAtIndex: i];
            if (widget.model.hidden) {
                continue;
            }
            ...
            if (((ViewM *) self.model).overflow == OverflowTypeVisible && [widget isKindOfClass: [ViewWidget class]]
